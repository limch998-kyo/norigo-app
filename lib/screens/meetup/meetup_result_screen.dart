...
String _buildMeetupShareUrl(dynamic state, String locale) {
  ...
  return Uri.parse(
    'https://norigo.app/$locale/result',
...
class _MeetupResultScreenState extends ConsumerState<MeetupResultScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final resultKey = [
      state.region,
      state.mode,
      state.filledStations.map((s) => s.id).join(','),
      result.stations.length,
      result.stations.first.station.id,
    ].join('|');
    if (_trackedResultKey != resultKey) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        ref
            ...
            .trackEvent(
              'meetup_result_viewed',
              payload: {
                'region': state.region,
                'mode': state.mode,
                'participantCount': state.filledStations.length,
                'stationIds': state.filledStations.map((s) => s.id).toList(),
                'resultCount': result.stations.length,
                'topStationId': result.stations.first.station.id,
                'topStationName': result.stations.first.station.name,
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return _StationCard(
                        ...
                        onVenueOutbound: (provider, venue, rec) {
                          ref
                              .read(trackingServiceProvider)
                              .trackEvent(
                                ...
                                payload: {
                                  'provider': provider,
                                  if (venue != null) 'venueName': venue.name,
...
class _StationCard extends StatelessWidget {
  ...
  final void Function(String provider, Venue? venue, RecommendedStation rec)?
  onVenueOutbound;
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final name =
        localNames[rec.station.id] ?? rec.station.localizedName(locale);
              ...
              Builder(
                builder: (context) {
                  ...
                  final mapCenter = LatLng(
                    allMapPoints.fold(0.0, (sum, p) => sum + p.latitude) /
                        allMapPoints.length,
                    allMapPoints.fold(0.0, (sum, p) => sum + p.longitude) /
                        allMapPoints.length,
                  );
                        ...
                        children: [
                          ...
                          MarkerLayer(
                            markers: [
                              ...
                              ...rec.distances
                                  .where(
                                    (d) => d.lat != 0 && d.estimatedMinutes > 0,
                                  )
                                  .map((d) {
                                    final pName =
                                        localNames[d.participantStationId] ??
                ...
                Row(
                  children: [
                    ...
                    Text(
                      tr(
                        locale,
                        ja: '周辺のお店 (${rec.venues.length}件)',
                        ko: '주변 맛집 (${rec.venues.length}개)',
                        en: 'Nearby (${rec.venues.length})',
                        zh: '附近餐厅 (${rec.venues.length}家)',
                        fr: 'À proximité (${rec.venues.length})',
                      ),
                ...
                if (locale == 'ja')
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () {
                        final stationName =
                            localNames[rec.station.id] ??
                            rec.station.localizedName('ja');
                        final query = Uri.encodeComponent(
                          '$stationName駅 レストラン',
                        );
                        final url =
                            'https://www.google.com/maps/search/$query/@${rec.station.lat},${rec.station.lng},16z';
                        onVenueOutbound?.call('google_maps', null, rec);
                        launchUrl(
                          Uri.parse(url),
                      ...
                      label: Text(
                        tr(
                          locale,
                          ko: '주변 맛집 검색 (Google Maps)',
                          en: 'Search restaurants (Google Maps)',
                          zh: '搜索附近餐厅 (Google Maps)',
                          fr: 'Rechercher des restaurants (Google Maps)',
                        ),
                        style: const TextStyle(fontSize: 13),
                      ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    if (venue.genre != null) _Badge(text: venue.genre!),
                ...
                if (venue.privateRoom ||
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      if (venue.wifi)
                        const _FeatureBadge(icon: Icons.wifi, label: 'WiFi'),
                    ],
                  ),
                ],
...
class _VoteButtonState extends State<_VoteButton> {
  ...
  Future<void> _createPoll() async {
    ...
    if (pollId != null && mounted) {
      ...
      Navigator.of(
        context,
      ).push(MaterialPageRoute(builder: (_) => VoteScreen(pollId: pollId)));
    } else {
      ...
      return SizedBox(
        ...
        child: OutlinedButton.icon(
          onPressed: () => Navigator.of(context).push(
            MaterialPageRoute(builder: (_) => VoteScreen(pollId: _pollId!)),
          ),
      ...
      return Center(
        child: Text(
          tr(
            ...
            en: 'No map data',
            zh: '无地图数据',
            fr: 'Aucune donnée cartographique',
          ),
        ),
      );
