...
/// Extract inner URL from /api/out wrapper
...
void main() {
  ...
  group('Budget: different night counts', () {
    test('1 night: 10000-30000 → Expedia price=67&price=200', () {
      ...
      expect(_innerUrl(providers[0].url), contains('price=67'));
      expect(_innerUrl(providers[0].url), contains('price=200'));
    });
    ...
    test('5 nights: 10000-30000 → Expedia price=333&price=1000', () {
      ...
      expect(_innerUrl(providers[0].url), contains('price=333'));
      expect(_innerUrl(providers[0].url), contains('price=1000'));
      // Hotels.com stays per-night regardless of nights
      expect(_innerUrl(providers[1].url), contains('price=67'));
      expect(_innerUrl(providers[1].url), contains('price=200'));
    });
  });
}
