...
({String checkIn, String checkOut}) _futureHotelDates() {
  final checkIn = DateTime.now().add(const Duration(days: 30));
  final checkOut = checkIn.add(const Duration(days: 2));
  return (checkIn: _dateKey(checkIn), checkOut: _dateKey(checkOut));
}
...
void main() {
  ...
  group('Station Search - Web Parity', () {
    test('returns station with id, name, lines', () async {
      ...
      expect(
        s.id,
        isNotEmpty,
        reason: 'Station must have id for recommend API',
      );
      expect(s.name, isNotEmpty);
      expect(s.lines, isNotEmpty, reason: 'Web shows line badges');
      print(
        '✓ Station: ${s.name} (${s.id}), lines: ${s.lines.take(3).join(", ")}',
      );
    });
    ...
    test('kanto and kansai regions work', () async {
      ...
      print(
        '✓ Kanto: ${kanto.length} results, Kansai: ${kansai.length} results',
      );
    });
  });
  ...
  group('Meetup Recommend - Web Parity', () {
    ...
    test('returns stations with all required fields', () async {
      ...
      expect(
        r.avgEstimatedMinutes,
        greaterThanOrEqualTo(0),
        reason: 'Avg minutes',
      );
      expect(
        r.maxEstimatedMinutes,
        greaterThanOrEqualTo(0),
        reason: 'Max minutes',
      );
      ...
      print(
        '✓ Meetup result: ${r.station.name}, score=${r.finalScore}, avg=${r.avgEstimatedMinutes}min',
      );
    });
    ...
    test('distances contain route segments', () async {
      ...
      final d = allDistances.firstWhere(
        (d) => d.distanceKm > 0 && d.route.isNotEmpty,
      );
      expect(
        d.participantStationName,
        isNotEmpty,
        reason: 'Web shows participant name',
      );
      expect(d.distanceKm, greaterThan(0), reason: 'Web shows distance in km');
      expect(
        d.route,
        isNotEmpty,
        reason: 'Web shows route bar per participant',
      );
      ...
      expect(
        seg.color,
        startsWith('#'),
        reason: 'Route color for visualization',
      );
      expect(seg.minutes, greaterThan(0), reason: 'Segment duration');
      print(
        '✓ Route: ${d.participantStationName} → ${seg.line} (${seg.color}, ${seg.minutes}min)',
      );
    });
    ...
    test('venues (Hot Pepper) with photo and details', () async {
      ...
      expect(
        venues,
        isNotEmpty,
        reason: 'Web shows venues when category selected',
      );
      ...
      print(
        '✓ Venue: ${v.name}, genre=${v.genre}, budget=${v.budget}, photo=${v.imageUrl != null}',
      );
    ...
    test('all 3 modes return different results', () async {
      final centroid = await api.getMeetupRecommendation(
        stationIds: [shibuyaId, shinjukuId],
        mode: 'centroid',
        region: 'kanto',
      );
      final minTotal = await api.getMeetupRecommendation(
        stationIds: [shibuyaId, shinjukuId],
        mode: 'minTotal',
        region: 'kanto',
      );
      final balanced = await api.getMeetupRecommendation(
        stationIds: [shibuyaId, shinjukuId],
        mode: 'balanced',
        region: 'kanto',
      );
      ...
      print(
        '✓ 3 modes: centroid=${centroid.stations.first.station.name}, minTotal=${minTotal.stations.first.station.name}, balanced=${balanced.stations.first.station.name}',
      );
    });
  });
  ...
  group('Stay Recommend - Web Parity', () {
    ...
    test('returns areas with all required fields', () async {
      ...
      print(
        '✓ Stay area: ${a.station.name}, score=${a.finalScore}, avg=${a.avgEstimatedMinutes}min',
      );
    });
    ...
    test('landmark distances with route data', () async {
      ...
      expect(
        d.estimatedMinutes,
        greaterThanOrEqualTo(0),
        reason: 'Web shows minutes',
      );

      // Route for transit (web shows RouteBar or "walking" if ≤1km)
      final hasRoute = ld.any((l) => l.route.isNotEmpty);
      print(
        '✓ Landmark distances: ${ld.map((l) => "${l.landmarkName}=${l.estimatedMinutes}min(${l.distanceKm}km)").join(", ")}',
      );
      print('  Has route data: $hasRoute');
      print(
        '  Walking (≤1km): ${ld.where((l) => l.distanceKm <= 1.0).map((l) => l.landmarkName).join(", ")}',
      );
    });
    ...
    test('reachable destinations present', () async {
      ...
      print(
        '✓ Reachable: ${rd.take(3).map((r) => "${r.name}=${r.minutes}min").join(", ")}',
      );
    });
  ...
  group('Hotels API - Web Parity', () {
    test('returns hotels with all Agoda fields', () async {
      ...
      expect(
        h.bookingUrl,
        isNotNull,
        reason: 'Booking URL (landingURL from API)',
      );
      print(
        '✓ Hotel: ${h.name}, rate=${h.formattedPrice}, score=${h.formattedRating}',
      );
    });
    ...
    test('amenity fields present', () async {
      ...
      print(
        '✓ Amenities: breakfast=${hasBreakfast ? "some" : "none"}, wifi=${hasWifi ? "some" : "none"}',
      );
    });
    ...
    test('discount (crossedOutRate) present for some hotels', () async {
      ...
      if (discounted.isNotEmpty) {
        ...
        print(
          '  Example: ${h.name} ${h.formattedCrossedOutPrice} → ${h.formattedPrice} (-${h.discountPercent}%)',
        );
      }
    });
