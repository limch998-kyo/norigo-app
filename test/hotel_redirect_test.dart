...
void main() {
  ...
  final testCases = [
    // Kanto
    (region: 'kanto', station: 'Shinjuku', lat: 35.69, lng: 139.70, locale: 'en'),
    (region: 'kanto', station: 'Shibuya', lat: 35.66, lng: 139.70, locale: 'ko'),
    // Kansai
    (region: 'kansai', station: 'Namba', lat: 34.67, lng: 135.50, locale: 'en'),
    // Kyushu
    (region: 'kyushu', station: 'Hakata', lat: 33.59, lng: 130.42, locale: 'en'),
    // CJK fallback
  ...
  for (final tc in testCases) {
    group('${tc.station} (${tc.region}, ${tc.locale})', () {
      late List<({String name, String url, Color color, Color textColor})> providers;

      setUpAll(() {
        providers = BookingProvider.buildMultiProviderUrls(
          locale: tc.locale, region: tc.region, stationName: tc.station,
      ...
      test('Expedia: /api/out → 302 → Expedia page with station', () async {
        ...
        expect(decoded, contains('expedia'), reason: 'Should redirect to expedia');
        expect(decoded, contains('Hotel-Search'), reason: 'Should be hotel search page');

        // CJK fallback check
        final hasCjk = RegExp(r'[\u3000-\u9FFF\uAC00-\uD7AF]').hasMatch(tc.station);
        if (hasCjk) {
          expect(decoded, contains('Tokyo') , reason: 'CJK should fallback to Tokyo/Seoul');
        } else {
          expect(decoded, contains(tc.station), reason: 'Station name should be in URL');
        }

        // Price params
        expect(decoded, contains('price='), reason: 'Should have price filter');
        expect(decoded, contains('affcid='), reason: 'Should have affiliate ID');
      });

      test('Hotels.com: /api/out → 302 → Hotels.com page with station + USD', () async {
        ...
        expect(decoded, contains('hotels.com'), reason: 'Should redirect to hotels.com');
        expect(decoded, contains('www.hotels.com'), reason: 'Should use www.hotels.com');
        expect(decoded, contains('currency=USD'), reason: 'Should force USD');
        expect(decoded, contains('siteid='), reason: 'Should have site ID');
        expect(decoded, contains('price='), reason: 'Should have price filter');
        expect(decoded, contains('affcid='), reason: 'Should have affiliate ID');
        expect(decoded, contains('sort=RECOMMENDED'), reason: 'Should sort by recommended');
      });

      test('Booking.com: /api/out → 302 → Booking.com page with 駅 + lat/lng', () async {
        if (providers.isEmpty) return;

        final res = await dio.get(providers[2].url);
        expect(res.statusCode, 302, reason: 'Booking.com /api/out should 302');

        final location = res.headers['location']?.first ?? '';
        final decoded = Uri.decodeComponent(location);
        print('  Booking.com → $decoded');

        expect(decoded, contains('booking.com'), reason: 'Should redirect to booking.com');
        expect(decoded, contains('searchresults'), reason: 'Should be search results page');
        expect(decoded, contains('駅'), reason: 'Should append 駅 to search');
        expect(decoded, contains('latitude='), reason: 'Should have latitude');
        expect(decoded, contains('longitude='), reason: 'Should have longitude');
        expect(decoded, contains('aid='), reason: 'Should have affiliate ID');
        expect(decoded, contains('nflt='), reason: 'Should have price filter');
      });
    });
  }
  ...
  group('Final page actually loads (follow redirects)', () {
    final followDio = Dio(BaseOptions(
    ...
    test('Booking.com final page returns 200', () async {
      ...
      try {
        ...
        expect(res.statusCode, lessThan(500), reason: 'Booking.com should not 500');
      } catch (e) {
